...
public class ExtractorFramesRare {
	...
	/**
	 * Add more feature for vietnamese unknown words
	 */
	...
	private static final Extractor cWordPrefixSyllable = new ExtractorCWordPrefixSyllable();
	private static final Extractor cWordSuffixSyllable = new ExtractorCWordSuffixSyllable();
	private static final Extractor cWordNumberOfSyllable = new ExtractorCWordNumberOfSyllable();
	private static final Extractor cWordCapitalizedSyllable = new ExtractorCWordCapitalizedSyllable();
	private static final Extractor cWordNumberOfCharacter = new ExtractorCWordNumberOfCharacter();
	private static final Extractor cWordAllUppercaseOrDigit = new ExtractorCWordAllUppercaseOrDigit();
	private static final Extractor cWordIsRepetitive = new ExtractorCWordIsRepetitive();
	private static final Extractor cWordIsMark = new ExtractorCWordIsMark();
	private static final Extractor cAllNumber = new ExtractorAllNumber();
	private static final Extractor dicTags = new ExtractorDicTags();
	
	
	private static final Extractor[] eFrames_vietnamese_unknowns = { cWordUppCase, cWordNumber, cWordDash, cNoLower,
														cAllCapitalized, cLetterDigitDash, cMidSentence,
														cWordStartUCase, cWordMidUCase, cUpperDigitDash,
														cCompany, cWordPrefixSyllable, cWordSuffixSyllable,
														cWordNumberOfSyllable, cWordCapitalizedSyllable,
														cWordNumberOfCharacter, cWordAllUppercaseOrDigit,
														cWordIsRepetitive, dicTags };
	...
	protected static Extractor[] getExtractorFramesRare(String identifier, TTags ttags) {
		...
		for (String arg : args) {
			if ("vietnameseunknowns".equalsIgnoreCase(arg)) {
				extrs.addAll(Arrays.asList(eFrames_vietnamese_unknowns));
			} else if ("naacl2003unknowns".equalsIgnoreCase(arg)) {
...
class RareExtractor extends Extractor {
	...
	protected static boolean allNumber(String s) {
	...
	static boolean isSign(String str) {
...
/**
* Extract feature if current word is repeatitive
*/
class ExtractorCWordIsRepetitive extends RareExtractor {

	private static final long serialVersionUID = -4462884231338976625L;
	
	@Override
	String extract(History h, PairsHolder pH) {
		String word = pH.getWord(0);
			...
			if (parser1.isValidVnSyllable() && parser2.isValidVnSyllable()){
				if (parser1.isValidVnSyllable() && parser2.isValidVnSyllable()){
					if (parser1.getNonToneSyll().equalsIgnoreCase(parser2.getNonToneSyll()) ||
							parser1.getRhyme().equalsIgnoreCase(parser2.getRhyme())){
						return "1";
					}
				}
			}
...
/**
* Extract feature if current word is punctuation mark
*/
class ExtractorCWordIsMark extends RareExtractor {

	private static final long serialVersionUID = 7256683094033391714L;

	public ExtractorCWordIsMark() {
	}
...
/**
* This extractor extracts tag of word in dictionary.
*/
class ExtractorDicTags extends RareExtractor {
	
	private static final long serialVersionUID = 2785028427623127306L;
...
//		String cp = "";
		...
		if (ExtractorFrames.word2dictags.containsKey(word)){
			List<String> tags = (List) ExtractorFrames.word2dictags.get(word);
...
/**
 * Extract prefix of word
 */
class ExtractorCWordPrefixSyllable extends RareExtractor
{
...
/**
 * Extract number of syllables of word
 */
...
/**
 * Extract first character of word is capitalized
 */
class ExtractorCWordCapitalizedSyllable extends RareExtractor {
	...
	String extract(History h, PairsHolder pH)
	{
		...
		for (String syllable : syllables) {
			if (syllable.length() > 0 && Character.isLowerCase(syllable.charAt(0))) {
				return "0";
			}
		}
		return "1";
...
/**
 * Extract feature if current word has only characters or digits
 */
class ExtractorCWordAllUppercaseOrDigit extends RareExtractor {
	private static final long serialVersionUID = 7594961218743402402L;
...
class ExtractorAllNumber extends RareExtractor {

	private static final long serialVersionUID = -3193597827895273084L;
	public ExtractorAllNumber() {
	}
