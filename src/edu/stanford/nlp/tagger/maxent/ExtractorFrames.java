...
public class ExtractorFrames {
	...
	private static final Extractor cWordPrevWord = new ExtractorTwoWords(-1);
	private static final Extractor cWordNextWord = new ExtractorTwoWords(0);
	private static final Extractor prevTag = new Extractor(-1, true);
	private static final Extractor prevTag2 = new Extractor(-2, true);
	...
	private static final Extractor nextTagWord = new ExtractorWordTag(0, 1);
	...
	// features for VIETNAMESE tagger
	private static final Extractor[] eFrames_vietnamese = { cWord, prevWord, prevWord2, nextWord, nextWord2,
															prevTag, nextTag, prevTwoTags, nextTwoTags, prevNextTag,
															prevTagWord, nextTagWord, cWordPrevWord, cWordNextWord };
	
	// features for 2005 SIGHAN tagger
	private static final Extractor[] eFrames_sighan2005 = { cWord, prevWord, prevWord2, nextWord, nextWord2, prevTag, prevTag2, new ExtractorContinuousTagConjunction(-2) };
	...
	private static final String DICTAGS_PATH = "JVnTagger/ComputerDict.txt";
	public static Map word2dictags = new HashMap<String, List>();
	// init dictags
	static {
		readDict();
	}
	...
	public static boolean readDict(){
		System.out.println("Reading word2tags dictionary...");
		try {
			 FileReader in = new FileReader(DICTAGS_PATH);
	...
	protected static Extractor[] getExtractorFrames(String arch) {
		...
		arch = arch.replaceAll("vnbidirectional", "words(-2,1),order(-2,1),biwords(-2,1),wordTag(0,-1),wordTag(0,1),wordTwoTags(0,-1,-2),twoTags(-1,1)");
...
//		arch = arch.replaceAll("vnbidirectional", "words(-2,2),order(-2,2),biwords(-2,2),wordTag(0,-1),wordTag(0,1),wordTwoTags(0,-1,-2),wordTwoTags(0,1,2)");
		
		
		ArrayList<Extractor> extrs = new ArrayList<Extractor>();
		List<String> args = StringUtils.valueSplit(arch, "[a-zA-Z0-9]*(?:\\([^)]*\\))?", "\\s*,\\s*");
		for (String arg : args) {
			if (arg.equalsIgnoreCase("vietnamese")) {
//				extrs.addAll(Arrays.asList(eFrames_vietnamese));
				...
				extrs.add(new ExtractorSpanishSemiauxiliaryTag());
					...
					arg.equalsIgnoreCase("vietnameseunknowns")) {
...
//	 */
//	static class ExtractorCWordIsRepetitive extends Extractor {
//
//		private static final long serialVersionUID = -1395836561583522793L;
//
//		@Override
...
//	 */
//	static class ExtractorDicTags extends Extractor {
//		
//		private static final long serialVersionUID = -8957475378388097055L;
//
//		@Override
//		String extract(History h, PairsHolder pH) {
//			String word = pH.getWord(0);
...
//				List<String> tags = (List) word2dictags.get(word);
//				
...
//	}
	...
	/**
	 * This extractor extracts two previous tags of current tag.
	 */
	static class ExtractorPrevTwoTags extends Extractor {
	...
	/**
	 * This extractor extracts two next tags of current tag.
	 */
	static class ExtractorNextTwoTags extends Extractor {
	...
	/**
	 * This extractor extracts previous tag and next tag of current tag.
	 */
	class ExtractorPrevTagNextTag
	  extends Extractor
	{
	...
	/**
	 * This extractor extracts three previous tags.
	 */
	static class ExtractorPrevThreeTags extends Extractor {
